...
int get_udr_options(UDR_Options* udr_options, int argc, char* argv[],
                    int rsync_arg_idx) {
  int ch;
  char* key_dir = NULL;
  ...
  while ((ch = getopt_long(rsync_arg_idx, argv, opts, long_options,
                           &option_index)) != -1) {
    switch (ch) {
      case 'P':
  ...
  if (udr_options->timeout < 1) {
    fprintf(
        stderr,
        "Please specify a timeout duration [-d timeout] greater than 0s.\n");
    exit(1);
  }

  // Finish setting up the key file path
  if (key_dir == NULL) {
    snprintf(udr_options->key_filename, PATH_MAX, "%s",
             udr_options->key_base_filename);
  } else {
    sprintf(udr_options->key_filename, "%s/%s", key_dir,
            udr_options->key_base_filename);
  }
  ...
  if (udr_options->verbose) {
    ...
    fprintf(stderr, "%s Local program: %s Remote program: %s Encryption: %d\n",
            ...
            udr_options->udr_program_dest, udr_options->encryption);
  }
  ...
  for (int i = rsync_arg_idx; i < argc; i++) {
    if (strcmp(argv[i], "-e") == 0 || strcmp(argv[i], "--rsh") == 0) {
      fprintf(stderr,
              "UDR ERROR: UDR overrides the -e, --rsh flag of rsync, so they "
...
void parse_host_username(char* source, char* username, char* host,
  ...
  if (at_loc != NULL) {
    ...
    if (host_len > PATH_MAX) host_len = PATH_MAX;
...
void get_host_username(UDR_Options* udr_options, int argc, char* argv[],
                       int rsync_arg_idx) {
  ...
  for (int i = argc - 2; i > rsync_arg_idx; i--) {
    ...
    if (argv[i][0] == '-') {
      ...
      if (src_num != 0) {
        if (strcmp(src_username, next_src_username) != 0 ||
            strcmp(src_host, next_src_host) != 0 ||
            src_double_colon != next_src_double_colon) {
          // have a problem
          fprintf(stderr,
                  "UDR ERROR: source must use the same host and username\n");
          exit(-1);
        }
      }
  ...
  if (src_remote == dest_remote) {
    fprintf(stderr,
            "UDR ERROR: UDR only does remote -> local or local -> remote "
            "transfers\n");
    exit(-1);
  }
