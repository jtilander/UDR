...
void *monitor_timeout(void *_arg) {
  ...
  while (1) {
    ...
    // round
    if (timeout_sem != 2) timeout_sem = 0;
  }
}
...
int run_sender(UDR_Options *udr_options, unsigned char *passphrase,
  ...
  if (0 !=
      getaddrinfo(udr_options->host, udr_options->port_num, &hints, &peer)) {
    cerr << "[udr sender] incorrect server/peer address. " << udr_options->host
         << ":" << udr_options->port_num << endl;
    return 1;
  }

  if (UDT::ERROR == UDT::connect(client, peer->ai_addr, peer->ai_addrlen)) {
    cerr << "[udr sender] connect: " << UDT::getlasterror().getErrorMessage()
         << endl;
    return 1;
  }
  ...
  while (ssize < n) {
    if (UDT::ERROR == (ss = UDT::send(client, cmd + ssize, n - ssize, 0))) {
      cerr << "[udr sender] Send:" << UDT::getlasterror().getErrorMessage()
           << endl;
      break;
    }
  ...
  sender_to_udt.fd =
      STDIN_FILENO;  // stdin of this process, from stdout of rsync
  sender_to_udt.id = 0;
  ...
  udt_to_sender.fd = STDOUT_FILENO;  // stdout of this process, going to stdin
                                     // of rsync, rsync defaults to set this is
                                     // non-blocking
  udt_to_sender.id = 1;
...
int run_receiver(UDR_Options *udr_options) {
  ...
  for (int port_num = udr_options->start_port;
       port_num <= udr_options->end_port; port_num++) {
    bad_port = false;
  ...
  if (bad_port) {
    fprintf(
        stderr,
        "[udr receiver] ERROR: could not bind to any port in range %d - %d\n",
        udr_options->start_port, udr_options->end_port);
    return 0;
  }
  ...
  if (UDT::ERROR == UDT::listen(serv, 10)) {
    cerr << "[udr receiver] listen: " << UDT::getlasterror().getErrorMessage()
         << endl;
    return 0;
  }
  ...
  if (udr_options->server_connect) {
    ...
    if (strlen(udr_options->server_config) > 0) {
      sprintf(rsync_cmd, "%s%s %s",
              "rsync --config=", udr_options->server_config,
              " --server --daemon .");
    } else {
  ...
  int rsync_pid = fork_execvp(udr_options->shell_program, sh_cmd,
  ...
  // all of our threads and exit as well  also going to check if either thread
  // is complete, if one is then the other should also be killed  bit of a hack
  // to deal with the pthreads
  while (true) {
