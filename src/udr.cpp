...
char* get_udr_cmd(UDR_Options* udr_options) {
  ...
  if (udr_options->verbose) strcat(udr_args, "-v");
  ...
  if (udr_options->server_connect) {
    ...
    sprintf(udr_args, "%s -a %d -b %d %s", udr_args, udr_options->start_port,
            udr_options->end_port, "-t rsync");
  }

  char* udr_cmd = (char*)malloc(strlen(udr_options->udr_program_dest) +
                                strlen(udr_args) + 3);
...
void print_version() { fprintf(stderr, "UDR version %s\n", version); }
...
int main(int argc, char* argv[]) {
  ...
  if (curr_options.version_flag) print_version();
  ...
  //}
    ...
    for (int i = 0; i < rsync_argc; i++) {
      if (curr_options.verbose)
        fprintf(stderr, "%s rsync arg[%d]: %s\n", curr_options.which_process, i,
                rsync_args[i]);

      // hack for when no directory is specified -- because strtok is lame,
      // probably should write own tokenizer, but this will do for now
    ...
    run_sender(&curr_options, passphrase, arguments.c_str(), rsync_argc,
               rsync_args);
     ...
     * curr_options.server_connect, curr_options.server is for the udr server */
    if (curr_options.server_connect) {
      ...
      int server_exists =
          get_server_connection(curr_options.host, curr_options.server_port,
                                udr_cmd, line, line_size);
    ...
    /* If not try ssh */
      ...
      fork_execvp(curr_options.ssh_program, ssh_argv, &sshparent_to_child,
                  &sshchild_to_parent);
      ...
      if (nbytes <= 0) {
        fprintf(stderr,
                "UDR ERROR: unexpected response from server, exiting.\n");
        exit(EXIT_FAILURE);
      }
    }
    ...
    if (curr_options.verbose) strcat(udr_rsync_args1, "-v ");

    strcat(udr_rsync_args1, "-s");

    const char* udr_rsync_args2 = "-p";

    rsync_argv[rsync_idx] = (char*)malloc(
        strlen(curr_options.udr_program_src) + strlen(udr_rsync_args1) +
        strlen(curr_options.port_num) + strlen(udr_rsync_args2) +
        strlen(curr_options.key_filename) + 6);
    sprintf(rsync_argv[rsync_idx], "%s %s %s %s %s",
            curr_options.udr_program_src, udr_rsync_args1,
            curr_options.port_num, udr_rsync_args2, curr_options.key_filename);
    ...
    pid_t local_rsync_pid = fork_execvp(curr_options.rsync_program, rsync_argv,
    ...
    do {
      pid_t w =
          waitpid(local_rsync_pid, &rsync_exit_status, WUNTRACED | WCONTINUED);
      if (w == -1) {
        perror("waitpid");
        exit(EXIT_FAILURE);
      }
